...
import { ActorLogicFrom, assign, createActor, EventObject, fromCallback, log, setup } from "xstate";
...
const terminalKeyboard = fromCallback(({ sendBack }) => {
  console.log("Loading hotkeys-js for terminal...");
  import("hotkeys-js").then(({ default: hk }) => {
    hk("alt+1", () => sendBack({ type: "TOGGLE" }))
  })
});
const dialog = fromCallback<EventObject, { dialog: HTMLDialogElement }>(({ input: { dialog }, sendBack }) => {
...
interface TerminalMounted { type: "TERMINAL_MOUNTED"; dialog: HTMLDialogElement; router: Router };
...
const app = appSetup.createMachine({
  ...
  on: {
    TERMINAL_MOUNTED: {
      actions: [
        log("Terminal mounted"),
        assign({
          terminal: ({ event, spawn }) => spawn(terminal, { input: { dialog: event.dialog, router: event.router } })
        })
      ],
    }
  }
